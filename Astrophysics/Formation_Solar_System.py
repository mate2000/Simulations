...
for i in range(Nplanets):
    ...
    Stars[-1].trail = curve(pos=[Stars[-1].pos],
                            color=colors[i % 6], radius=Rtrail)
    mass = Msun*r**3/Rsun**3
    v0 = sqrt(G*Msun/rad)
    px = -1.0*mass*cos(theta)*v0
    py = 0.0
    pz = mass*sin(theta)*v0
    poslist.append((x, y, z))
    plist.append((px, py, pz))
    mlist.append(mass)
    rlist.append(r)
...
while 1:
    ...
    try:  # numpy
        ...
        rmag = sqrt(reduce(r*r, -1))  # star-to-star scalar distances
